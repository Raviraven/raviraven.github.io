...
function getPaginationHtml(totalPages, currentPage){
    ...
    let innerHtml = ``;
    innerHtml += createSinglePageSpan('<', 'projects__pagination--arrow', 'previousPage();');
    ...
    for(let i=0; i<totalPages; i++)
    {
        ...
        innerHtml += createSinglePageSpan(i+1, cssClass, `loadPageByNumber(${i+1});`);
    }

    innerHtml += createSinglePageSpan('>', 'projects__pagination--arrow', 'nextPage();');
    return innerHtml;
}
